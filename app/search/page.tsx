...
import { fetchAllSitters } from '@/lib/actions'
...
const Search = async({ searchParams: { locationM, endcursor } }: Props) => {//the page is showed when the user enters a location in the search bar
  ...
  const data =locationM? await fetchAllSitters(locationM, endcursor) as SitterSearch:await fetchAllSitters( undefined,endcursor) as SitterSearch //get all the sitters that have the searched location
