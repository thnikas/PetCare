...
const List = ({data,sitterData}:Props) => {//list of the user cards that is showed based on the search criteria
  ...
  return (
    <div className='justify-center flex pt-8 '  >
      ...
      <Link href={`/sitterPr/${data.id}`} className={`w-[600px] h-[12rem] relative bg-white rounded-[30px] border border-stone-300 containerProfile`} style={{height:size}} onClick={()=>setLoader(true)} >
        <img className="w-[11.5rem] h-[11.9rem] left-0 top-0 absolute imageUser " src={img} />
        <div className="left-[527px] top-[10px] absolute text-slate-500 text-base font-normal userPrice">{data.moneyD} €    
          <div className="w-[60px] h-5   absolute text-slate-500 text-sm font-normal userPrice">/day</div>
        </div>
